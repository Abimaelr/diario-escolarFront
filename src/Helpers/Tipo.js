...
export default function (t, s, markBolC, markBolN, action, index) {
    const id = action === 'c' ? index : s.alunoId;

    if (t === 'ci')
        return(
            <select name="nota" onChange={ (event) => markBolC(s.alunoId, event)} id={ id } >
    ...
    if(t === 'n')
        return <input onChange={ (event) => markBolN(s.alunoId, event) }type="number" min="0" defaultValue="0" max="10.0" id={ id } />
}
